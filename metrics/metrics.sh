...
sudo docker run -p 9100:9100 -d --name node_exporter --net=host -v $PWD/:/etc/config prom/node-exporter --path.rootfs=/etc/config
echo "running grafana on port 3000"
sudo docker run -d -p 3000:3000 --name grafana --net=host grafana/grafana
